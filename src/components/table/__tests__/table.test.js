...
describe('Table Component', () => {
  ...
  it('should allow expandable row content', () => {
    ...
    expect(component.find('tr.pf-v6-c-table__expandable-row')).toMatchSnapshot('no expanded row');

    const input = component.find('td.pf-v6-c-table__toggle button');
    component.fireEvent.click(input);

    expect(mockOnExpand.mock.calls).toMatchSnapshot('expand row event');
    expect(component.find('tr.pf-v6-c-table__expandable-row')).toMatchSnapshot('expanded row');
  });
  ...
  it('should allow expandable cell content', () => {
    ...
    const component = renderComponent(<Table {...props} />);
    expect(component.find('tr.pf-v6-c-table__expandable-row')).toMatchSnapshot('no expanded cell');

    const input = component.find('button');
    component.fireEvent.click(input);

    expect(mockOnExpand.mock.calls).toMatchSnapshot('expand cell event');
    expect(component.find('tr.pf-v6-c-table__expandable-row')).toMatchSnapshot('expanded cell');
  });
  ...
  it('should allow sortable content', () => {
    ...
    const input = component.find('th button.pf-v6-c-table__button');
    component.fireEvent.click(input);
